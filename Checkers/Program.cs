...
{    
    ...
    static void Main(string[] args) { // MAIN method
        ...
        do {
            log = "";
            ExecuteInstruction(GetInput().ToLower());
            LogIt(logPath, log);
            Draw();
    ...
    static public string GetInput() { // Request an input and verify its compliance
        string? stringInput = Console.ReadLine();
        if (stringInput == null) stringInput = "";
        return stringInput;
    }

    static public void ExecuteInstruction(string stringInput) { // Assignes token based on the input
        targetCoordinateX = 0;
        targetCoordinateY = 0;
        inputCoordinates = (0,0);

        if (stringInput.Length == 2 && InterpretCoordinates(stringInput) && (inputCoordinates.Item1 != 0 || inputCoordinates.Item2 != 0)) {
    ...
    static public bool InterpretCoordinates(string stringInput) { // translates the input string into a clean token
        for (int i = 0; i < 2; i++) {
            switch (stringInput[i]) {
                case '1':
                    inputCoordinates.Item1 = highestX;
                    break;
                case '2':
                    inputCoordinates.Item1 = highestX - 2;
                    break;
                case '3':
                    inputCoordinates.Item1 = highestX - 4;
                    break;
                case '4':
                    inputCoordinates.Item1 = highestX - 6;
                    break;
                case '5':
                    inputCoordinates.Item1 = highestX - 8;
                    break;
                case '6':
                    inputCoordinates.Item1 = highestX - 10;
                    break;
                case '7':
                    inputCoordinates.Item1 = highestX - 12;
                    break;
                case '8':
                    inputCoordinates.Item1 = lowestX;
                    break;
                case 'a':
                    inputCoordinates.Item2 = lowestY;
                    break;
                case 'b':
                    inputCoordinates.Item2 = highestY - 12;
                    break;
                case 'c':
                    inputCoordinates.Item2 = highestY - 10;
                    break;
                case 'd':
                    inputCoordinates.Item2 = highestY - 8;
                    break;
                case 'e':
                    inputCoordinates.Item2 = highestY - 6;
                    break;
                case 'f':
                    inputCoordinates.Item2 = highestY - 4;
                    break;
                case 'g':
                    inputCoordinates.Item2 = highestY - 2;
                    break;
                case 'h':
                    inputCoordinates.Item2 = highestY;
                    break;
                default:
                    inputCoordinates.Item1 = 0;
                    inputCoordinates.Item2 = 0;
                    break;
            }
        }
        targetCoordinateX = inputCoordinates.Item1;
        targetCoordinateY = inputCoordinates.Item2;

        if (inputCoordinates.Item1 == 0 || inputCoordinates.Item2 == 0) return false;
        else return true;
    }

    static public void GameMaster(string thePlayer, string theSelectedPlayer, string premiumFigure, string SelectedPremiumFigure, string theOtherPlayer, string theOtherPremium, string stringInput) { // Contains the game logic and rules
        if (board[targetCoordinateX, targetCoordinateY] == thePlayer || board[targetCoordinateX, targetCoordinateY] == premiumFigure) {
            ...
            if (isPremiumSelected == false) {
                if (CheckMoveLegality(false, false, theOtherPlayer, theOtherPremium)) {
                ...
                if (CheckMoveLegality(false, true, theOtherPlayer, theOtherPremium)) {
                    board[playerCoordinateX, playerCoordinateY] = blackTile;
            ...
            if (isPremiumSelected == false) {
                if (CheckMoveLegality(true, false, theOtherPlayer, theOtherPremium)) {
                    board[playerCoordinateX, playerCoordinateY] = blackTile;
                ...
                if (CheckMoveLegality(true, true, theOtherPlayer, theOtherPremium)) {
                    board[playerCoordinateX, playerCoordinateY] = blackTile;
                    board[targetCoordinateX, targetCoordinateY] = blackTile;
    ...
    static public bool CheckMoveLegality(bool checkBehind, bool isAllowedBack, string theOtherPlayer, string theOtherPremium) { // checks if the changes are applicable
    ...
    static public void Draw() { // Universal method used to draw the board or the menu
        if (drawGUI) {
            if (clearConsole) Console.Clear(); // For debugging
            if (drawBoard) {
                for (int i = 1; i < 20; i++) {
                    for (int j = 0; j < 21; j++) {
                        if (i == 18 || j == 3) continue; // This exception is created so the nonRendered value isn't drawn
    ...
    static public bool CheckForVictory() { // Scans the board and saves the positions of the pawns
        if (drawBoard) {
            ScanBoard();
    ...
    static public string ScanBoard(bool saveGame = false, bool loadGame = false, string saveData = "") { // finds the positions of the pawns
    ...
    static string ManageLogFiles() {
        ...
        string filePath;
        string content = @"                        --- Log File ---                        
            INPUT          INSTRUCTION         OUTCOME          
            ____________________________________________________ 
  ENTRY 0   |RUN / START   |(20,0 )            |Game started   | 
";
        
        for (int i = 1; i <= 256; i++) {
            filePath = Path.Combine(basePath, "Logs", "log" + i + ".txt");

            if (File.Exists(filePath)) {
                continue;
            } else {
                using (File.CreateText(filePath)) {};
                LogIt(filePath, content);
                return filePath;
            }
        }

        filePath = Path.Combine(basePath, "Logs", "LogXXX.txt");
